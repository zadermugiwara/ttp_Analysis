...
const SampleCfg SIG = {"root/Tt1M1600.root","mrecoil_isolated_toplikes_rec_missE_cut","dh_sig","pdf_sig",0.00024646};
...
static void PlotBestWindow(double L, double R, double bestZ)
{
    ...
    c1.SaveAs("bestWindow_invMass.png");
    ...
    c2.SaveAs("bestWindow_ratio.png");

    // --- also save both canvases to a ROOT file so they can be reused interactively
    TFile fout("bestWindow_canvases.root", "RECREATE");
    c1.Write();   // writes as object named c_bestTop
    c2.Write();   // writes as object named c_bestRatio
    fout.Close();
    std::cout << "\n>>> Resonance diagnostics in best slice\n";
auto* meanVar  = ws.var("m0");
auto* widthVar = ws.var("sig");
std::cout << "peak = "  << meanVar->getVal()
          << " ± "      << meanVar->getError()  << " GeV,  "
          << "width = " << widthVar->getVal()
          << " ± "      << widthVar->getError() << " GeV\n";
}
